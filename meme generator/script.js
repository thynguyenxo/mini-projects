...
function showQuote() {
  ...
  const wisdomQuote = document.createElement('p');
  const wisdomAuthor = document.createElement('p');
  wisdomQuote.textContent = randomQuote.quote;
  wisdomAuthor.textContent = '- ' + randomQuote.author;
  clearAll();
  
  const container = document.querySelector('.quote-content');
  container.appendChild(wisdomQuote);
  container.appendChild(wisdomAuthor);
}
...
function showRiddle() {
  ...
  const {question, answer} = randomRiddle; //destructuring Objects using ES6
...
function revealAnswers() {
	const riddleContainer = document.querySelector('.riddle-content');
	const riddleQuestion = riddleContainer.querySelector('p');
	const riddleAnswer = document.querySelector('#riddle-answer');
	
	if (riddleQuestion && riddleAnswer.hidden) {
		riddleAnswer.hidden = false;
	} else if (riddleQuestion) {
		alert("The riddle and the answer are already revealed.");
	} else {
		alert("There is no riddle. Click to generate one.");
	}
}
